...
;********************************************************************************************************
    ...
    EXPORT    OSCtxSw
    EXPORT    OSIntCtxSw
;    EXPORT    OS_CPU_PendSVHandler                              ; 注释掉
...
NVIC_INT_CTRL   EQU     0xE000ED04  ; 中断控制寄存器，具体请参考《Cortex-M3权威指南(中文).pdf》第284页内容
NVIC_SYSPRI2    EQU     0xE000ED20  ; 系统优先级寄存器（2），具体请参考《Cortex-M3权威指南(中文).pdf》第286页内容
;NVIC_SYSPRI14   EQU     0xE000ED22  ; 系统优先级寄存器（14），具体请参考《Cortex-M3权威指南(中文).pdf》第286页内容


NVIC_PENDSV_PRI EQU     0xFFFF0000  ; PendSV中断和系统节拍中断(都为最低，0xff)
NVIC_PENDSVSET  EQU     0x10000000  ; 软件触发中断值
